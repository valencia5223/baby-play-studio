...
class BabySoundEngine {
  ...
  async decodeAudio(arrayBuffer) {
    ...
    return new Promise((resolve) => {
      let isSettled = false;
      // 📱 iOS Safari AudioContext suspended 상태 시 decodeAudioData 무한 프리징 방지용 안전 타이머
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 🌙 브람스 자장가: 순수 JavaScript PCM 합성 → WAV Blob → HTML5 Audio 루프
  //    Web Audio BufferSource/OfflineAudioContext를 일절 사용하지 않음.
  //    HTML5 <audio> 태그의 loop 재생은 아이패드 Safari에서 가장 안정적.
  // ═══════════════════════════════════════════════════════════════════════
  startLullaby() {
    this.stopLullaby();
    this.init();
    ...
    // 이미 생성된 오디오가 있으면 즉시 재생
    if (this._lullabyAudio) {
      try {
        this._lullabyAudio.currentTime = 0;
        this._lullabyAudio.play().catch(() => {});
      } catch (e) { }
      return;
    }

    // 최초 1회: 순수 JavaScript로 PCM 파형 합성 → WAV Blob 생성
    try {
      const wavBuffer = this._generateLullabyWav();
      ...
      audio.play().then(() => {
        this._lullabyAudio = audio;
        this._lullabyBlobUrl = blobUrl;
      }).catch(() => {
        // HTML5 Audio 실패 시 fallback 없음 (조용히 실패)
        URL.revokeObjectURL(blobUrl);
      });
    } catch (e) { }
  }

  // 브람스 자장가 전체를 순수 JavaScript 사인파 합성으로 PCM 파형 생성 → WAV 파일로 패킹
  _generateLullabyWav() {
    const sampleRate = 44100;

    // 자장가 악보: f=주파수(Hz), d=박자(초), b=베이스 배열
    const score = [
    ...
    const totalDur = totalMelodyDur + 3.5; // 마지막 잔향
    const numSamples = Math.ceil(sampleRate * totalDur);
    const samples = new Float32Array(numSamples);

    const TWO_PI = 2 * Math.PI;

    // 각 음의 사인파 + 옥타브 배음 + 베이스 합성
    let noteStart = 0;
    score.forEach(note => {
      const startIdx = Math.floor(noteStart * sampleRate);
      const bellDur = 2.6;
      const bellSamples = Math.floor(bellDur * sampleRate);
      ...
      for (let i = 0; i < bellSamples; i++) {
        ...
        // 엔벨로프: 40ms 부드러운 어택 + 지수 감쇠
        let env;
        if (elapsed < 0.04) {
          env = elapsed / 0.04;
        } else {
          env = Math.exp(-(elapsed - 0.04) * 1.8);
        }

        // 기본 사인파
        const phase = TWO_PI * note.f * idx / sampleRate;
        samples[idx] += Math.sin(phase) * 0.28 * env;
        ...
        if (elapsed < 1.5) {
          ...
          if (elapsed < 0.03) {
            ...
            hEnv = Math.exp(-(elapsed - 0.03) * 3.0);
          }
          samples[idx] += Math.sin(phase * 2) * 0.06 * hEnv;
        }
      }
      ...
      if (note.b && note.b.length > 0) {
        note.b.forEach((bf, bi) => {
          const bStartIdx = Math.floor((noteStart + bi * 0.12) * sampleRate);
          const bassDur = 3.0;
          const bassSamples = Math.floor(bassDur * sampleRate);
          for (let i = 0; i < bassSamples; i++) {
            const idx = bStartIdx + i;
            if (idx >= numSamples) break;
            const elapsed = i / sampleRate;
            let env;
            if (elapsed < 0.04) {
              ...
              env = Math.exp(-(elapsed - 0.04) * 1.5);
            }
            const phase = TWO_PI * bf * idx / sampleRate;
            samples[idx] += Math.sin(phase) * 0.12 * env;
          }
        });
      }
    ...
    // 간단한 로우패스 (3-sample 이동 평균) → 따뜻하고 부드러운 톤
    const smooth = new Float32Array(numSamples);
    for (let i = 1; i < numSamples - 1; i++) {
      smooth[i] = (samples[i - 1] + samples[i] * 2 + samples[i + 1]) / 4;
    }
    smooth[0] = samples[0];
    smooth[numSamples - 1] = samples[numSamples - 1];

    // Float32 → 16-bit PCM WAV 변환
    return this._samplesToWav(smooth, sampleRate);
  }

  // Float32 PCM 샘플 → WAV 파일 ArrayBuffer 변환
  _samplesToWav(samples, sampleRate) {
...
function XylophoneChoirView() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '10px', userSelect: 'none' }}>
      ...
      {/* 4. 🎹🥁 실제 전자 드럼키보드 콘솔 (All-in-One Electronic Drum & Keyboard Console) */}
      <div className="drum-console-body" style={{
        flex: 1,
        minHeight: viewMode === 'both' ? '350px' : (viewMode === 'drums' ? '320px' : '230px'),
        padding: '12px 16px 14px',
        display: 'flex', flexDirection: 'column',
        position: 'relative', overflow: 'hidden',
        border: '4.5px solid #334155',
        boxShadow: 'inset 0 3px 6px rgba(255, 255, 255, 0.18), 0 14px 34px rgba(0, 0, 0, 0.45)'
      }}>
        {/* 콘솔 모서리 크롬 나사 4개 */}
        <div style={{ position: 'absolute', top: '8px', left: '10px', width: '8px', height: '8px', borderRadius: '50%', background: '#94a3b8', boxShadow: 'inset 0 1px 2px #fff, 0 1px 3px rgba(0,0,0,0.5)' }} />
        <div style={{ position: 'absolute', top: '8px', right: '10px', width: '8px', height: '8px', borderRadius: '50%', background: '#94a3b8', boxShadow: 'inset 0 1px 2px #fff, 0 1px 3px rgba(0,0,0,0.5)' }} />
        <div style={{ position: 'absolute', bottom: '8px', left: '10px', width: '8px', height: '8px', borderRadius: '50%', background: '#94a3b8', boxShadow: 'inset 0 1px 2px #fff, 0 1px 3px rgba(0,0,0,0.5)' }} />
        <div style={{ position: 'absolute', bottom: '8px', right: '10px', width: '8px', height: '8px', borderRadius: '50%', background: '#94a3b8', boxShadow: 'inset 0 1px 2px #fff, 0 1px 3px rgba(0,0,0,0.5)' }} />

        {/* 🌟 A. 상단 드럼 & 심벌즈 섹션 (드럼 모드 또는 전체 모드) */}
        {(viewMode === 'drums' || viewMode === 'both') && (
          <div style={{
            display: 'flex', flexDirection: 'column',
            flex: viewMode === 'drums' ? 1 : 'none',
            marginBottom: viewMode === 'both' ? '6px' : '0'
          }}>
            {/* 황금 심벌즈 2개 & 중앙 브랜드 배지 */}
            <div style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '0 16px', zIndex: 10, marginBottom: viewMode === 'drums' ? '12px' : '4px'
            }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                ...
                <div style={{
                  marginTop: '4px', background: activeDrumId === 'crash' ? '#facc15' : '#0f172a',
              ...
              {/* 가운데 브랜드 배지 */}
              <div style={{
                textAlign: 'center', zIndex: 5, padding: '5px 14px', borderRadius: '16px',
                background: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(8px)',
                border: '1.5px solid #475569', boxShadow: 'inset 0 2px 5px rgba(0,0,0,0.6)'
              }}>
                <div style={{ fontSize: '0.88rem', fontWeight: 900, color: '#fde047', letterSpacing: '0.5px' }}>
                  🥁 ELECTRONIC DRUM KEYBOARD 🎹
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginTop: '2px' }}>
                  <div className="speaker-grille-dots" style={{ width: '38px', height: '8px', borderRadius: '3px', opacity: 0.7 }} />
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 800 }}>BABY PLAY STUDIO</span>
                  <div className="speaker-grille-dots" style={{ width: '38px', height: '8px', borderRadius: '3px', opacity: 0.7 }} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                ...
                <div style={{
                  marginTop: '4px', background: activeDrumId === 'ride' ? '#fde047' : '#0f172a',
            ...
            {/* 🥁 5구 전자 드럼 패드 아크 (스네어, 하이톰, 베이스킥, 로우톰, 플로어톰) */}
            <div style={{
        ...
        {/* 🎛️ B. 중앙 조작 패널 스트립 (입체 스피커 그릴 & LED 비트 메터) */}
        {viewMode === 'both' && (
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '4px 12px', margin: '4px 0 8px', borderRadius: '12px',
            background: 'linear-gradient(180deg, #0f172a 0%, #1e293b 100%)',
            border: '1.5px solid #334155', boxShadow: 'inset 0 2px 5px rgba(0,0,0,0.6)'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div className="speaker-grille-dots" style={{ width: '70px', height: '18px', borderRadius: '6px', opacity: 0.8 }} />
              <span style={{ fontSize: '0.68rem', fontWeight: 900, color: '#64748b' }}>SPEAKER L</span>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ display: 'flex', gap: '3px', alignItems: 'flex-end', height: '16px' }}>
                {[6, 12, 16, 10, 14, 8, 15, 11].map((h, i) => (
                  <div
                    key={i}
                    style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontSize: '0.68rem', fontWeight: 900, color: '#64748b' }}>SPEAKER R</span>
              <div className="speaker-grille-dots" style={{ width: '70px', height: '18px', borderRadius: '6px', opacity: 0.8 }} />
            </div>
          </div>
        )}

        {/* 🎹 C. 8음계 실로폰/피아노 일체형 건반 영역 */}
        {(viewMode === 'xylophone' || viewMode === 'both') && (
          <div style={{
            height: viewMode === 'both' ? '150px' : '230px',
            background: 'linear-gradient(180deg, #0f172a 0%, #1e293b 100%)',
            borderRadius: '20px', padding: viewMode === 'both' ? '8px 12px' : '14px 18px',
            border: '3px solid #334155',
            boxShadow: 'inset 0 4px 10px rgba(0,0,0,0.6), 0 2px 6px rgba(255,255,255,0.05)',
...
// =============================================================================
// 🌙 2. 동물 친구들 코 잘 시간 (귀여운 애니메이션 동물 & 실제 퀼팅 이불 덮기)
// =============================================================================

// 🐰 베개에 누워있는 아기 토끼 SVG 애니메이션 컴포넌트
function SleepingRabbitIllustration({ isAsleep, hasBlanket }) {
  return (
    <svg viewBox="0 0 200 160" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
      <g className={isAsleep ? 'animal-breathing' : ''}>
        {/* 토끼 몸체 (이불 속에 들어갈 포근한 하얀 털) */}
        <ellipse cx="100" cy="115" rx="55" ry="38" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2.5" />

        {/* 쫑긋 귀 2개 (왼쪽/오른쪽) */}
        <g className="animal-ear-anim" style={{ transformOrigin: '75px 65px' }}>
          <ellipse cx="75" cy="38" rx="14" ry="36" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2.5" transform="rotate(-12 75 38)" />
          <ellipse cx="75" cy="38" rx="7.5" ry="24" fill="#fce7f3" transform="rotate(-12 75 38)" />
        </g>
        <g className="animal-ear-anim" style={{ transformOrigin: '125px 65px' }}>
          <ellipse cx="125" cy="38" rx="14" ry="36" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2.5" transform="rotate(12 125 38)" />
          <ellipse cx="125" cy="38" rx="7.5" ry="24" fill="#fce7f3" transform="rotate(12 125 38)" />
        </g>

        {/* 동글동글 토끼 얼굴 */}
        <ellipse cx="100" cy="80" rx="46" ry="38" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2.5" />

        {/* 발그레 핑크 볼터치 */}
        <ellipse cx="68" cy="88" rx="10" ry="7" fill="#f472b6" opacity="0.6" className="animal-blush-anim" />
        <ellipse cx="132" cy="88" rx="10" ry="7" fill="#f472b6" opacity="0.6" className="animal-blush-anim" />

        {/* 눈: 이불을 덮고 잠들었을 때는 감은 눈(︶ ︶), 깨어있을 때는 초롱초롱 뜬 눈 */}
        {isAsleep ? (
          <g>
            <path d="M 72 78 Q 80 85 88 78" fill="none" stroke="#475569" strokeWidth="3" strokeLinecap="round" />
            <path d="M 112 78 Q 120 85 128 78" fill="none" stroke="#475569" strokeWidth="3" strokeLinecap="round" />
          </g>
        ) : (
          <g>
            <ellipse cx="80" cy="78" rx="6" ry="7.5" fill="#1e293b" />
            <circle cx="78" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="82" cy="80.5" r="1.1" fill="#ffffff" />
            <ellipse cx="120" cy="78" rx="6" ry="7.5" fill="#1e293b" />
            <circle cx="118" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="122" cy="80.5" r="1.1" fill="#ffffff" />
          </g>
        )}

        {/* 귀여운 Y자 코와 입 */}
        <ellipse cx="100" cy="85" rx="4.5" ry="3.5" fill="#f472b6" />
        <path d="M 100 88 L 100 93 M 100 93 Q 94 97 90 94 M 100 93 Q 106 97 110 94" fill="none" stroke="#475569" strokeWidth="2.5" strokeLinecap="round" />

        {/* 앙증맞은 두 앞발 (이불 깃을 꼭 쥐고 있는 포즈) */}
        <g style={{ zIndex: 12 }}>
          <ellipse cx="78" cy="118" rx="11" ry="9" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2" />
          <ellipse cx="122" cy="118" rx="11" ry="9" fill="#ffffff" stroke="#fbcfe8" strokeWidth="2" />
        </g>
      </g>
    </svg>
  );
...
// 🐶 베개에 누워있는 아기 강아지 SVG 애니메이션 컴포넌트
function SleepingPuppyIllustration({ isAsleep, hasBlanket }) {
  return (
    <svg viewBox="0 0 200 160" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
      <g className={isAsleep ? 'animal-breathing' : ''}>
        {/* 강아지 몸체 */}
        <ellipse cx="100" cy="115" rx="55" ry="38" fill="#fef3c7" stroke="#fde68a" strokeWidth="2.5" />

        {/* 처진 부드러운 강아지 귀 2개 */}
        <g className="animal-ear-anim" style={{ transformOrigin: '65px 65px' }}>
          <path d="M 68 62 C 45 65 42 98 62 104 C 74 106 78 85 75 66 Z" fill="#d97706" opacity="0.9" />
        </g>
        <g className="animal-ear-anim" style={{ transformOrigin: '135px 65px' }}>
          <path d="M 132 62 C 155 65 158 98 138 104 C 126 106 122 85 125 66 Z" fill="#d97706" opacity="0.9" />
        </g>

        {/* 동글 강아지 얼굴 */}
        <ellipse cx="100" cy="80" rx="46" ry="38" fill="#fffbeb" stroke="#fde68a" strokeWidth="2.5" />

        {/* 이마의 귀여운 반점 */}
        <ellipse cx="88" cy="62" rx="14" ry="10" fill="#fcd34d" opacity="0.65" />

        {/* 발그레 볼터치 */}
        <ellipse cx="68" cy="88" rx="10" ry="7" fill="#fb7185" opacity="0.6" className="animal-blush-anim" />
        <ellipse cx="132" cy="88" rx="10" ry="7" fill="#fb7185" opacity="0.6" className="animal-blush-anim" />

        {/* 눈: 이불을 덮고 잠들었을 때는 감은 눈(︶ ︶), 깨어있을 때는 초롱초롱 뜬 눈 */}
        {isAsleep ? (
          <g>
            <path d="M 72 78 Q 80 84 88 78" fill="none" stroke="#334155" strokeWidth="3" strokeLinecap="round" />
            <path d="M 112 78 Q 120 84 128 78" fill="none" stroke="#334155" strokeWidth="3" strokeLinecap="round" />
          </g>
        ) : (
          <g>
            <ellipse cx="80" cy="78" rx="6" ry="7.5" fill="#334155" />
            <circle cx="78" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="82" cy="80.5" r="1.1" fill="#ffffff" />
            <ellipse cx="120" cy="78" rx="6" ry="7.5" fill="#334155" />
            <circle cx="118" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="122" cy="80.5" r="1.1" fill="#ffffff" />
          </g>
        )}
        ...
        {/* 촉촉한 밤톨 코와 미소 입 */}
        <ellipse cx="100" cy="85" rx="6" ry="4.5" fill="#334155" />
        <path d="M 100 90 L 100 94 M 100 94 Q 94 98 90 95 M 100 94 Q 106 98 110 95" fill="none" stroke="#334155" strokeWidth="2.5" strokeLinecap="round" />

        {/* 앞발 2개 */}
        <g style={{ zIndex: 12 }}>
          <ellipse cx="78" cy="118" rx="11" ry="9" fill="#fffbeb" stroke="#fde68a" strokeWidth="2" />
          <ellipse cx="122" cy="118" rx="11" ry="9" fill="#fffbeb" stroke="#fde68a" strokeWidth="2" />
        </g>
      </g>
    </svg>
  );
...
// 🐱 베개에 누워있는 아기 고양이 SVG 애니메이션 컴포넌트
function SleepingKittenIllustration({ isAsleep, hasBlanket }) {
  return (
    <svg viewBox="0 0 200 160" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
      <g className={isAsleep ? 'animal-breathing' : ''}>
        {/* 고양이 몸체 */}
        <ellipse cx="100" cy="115" rx="55" ry="38" fill="#ffedd5" stroke="#fed7aa" strokeWidth="2.5" />

        {/* 쫑긋 뾰족 귀 2개 */}
        <g className="animal-ear-anim" style={{ transformOrigin: '70px 65px' }}>
          <polygon points="60,65 72,32 90,60" fill="#fb923c" stroke="#fed7aa" strokeWidth="2" />
          <polygon points="65,62 73,40 85,58" fill="#fce7f3" />
        </g>
        <g className="animal-ear-anim" style={{ transformOrigin: '130px 65px' }}>
          <polygon points="140,65 128,32 110,60" fill="#fb923c" stroke="#fed7aa" strokeWidth="2" />
          <polygon points="135,62 127,40 115,58" fill="#fce7f3" />
        </g>

        {/* 둥근 고양이 얼굴 */}
        <ellipse cx="100" cy="80" rx="46" ry="38" fill="#fff7ed" stroke="#fed7aa" strokeWidth="2.5" />

        {/* 치즈 태비 줄무늬 */}
        <path d="M 94 48 L 100 58 L 106 48" fill="none" stroke="#ea580c" strokeWidth="2.5" opacity="0.6" strokeLinecap="round" />
        <path d="M 85 53 L 91 62" fill="none" stroke="#ea580c" strokeWidth="2" opacity="0.5" strokeLinecap="round" />
        <path d="M 115 53 L 109 62" fill="none" stroke="#ea580c" strokeWidth="2" opacity="0.5" strokeLinecap="round" />

        {/* 발그레 볼터치 */}
        <ellipse cx="68" cy="88" rx="10" ry="7" fill="#f43f5e" opacity="0.55" className="animal-blush-anim" />
        <ellipse cx="132" cy="88" rx="10" ry="7" fill="#f43f5e" opacity="0.55" className="animal-blush-anim" />
        ...
        {isAsleep ? (
          ...
          <g>
            <ellipse cx="80" cy="78" rx="6" ry="7.5" fill="#0f172a" />
            <circle cx="78" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="82" cy="80" r="1.2" fill="#38bdf8" opacity="0.9" />
            <ellipse cx="120" cy="78" rx="6" ry="7.5" fill="#0f172a" />
            <circle cx="118" cy="75.5" r="2.3" fill="#ffffff" />
            <circle cx="122" cy="80" r="1.2" fill="#38bdf8" opacity="0.9" />
          </g>
        )}

        {/* 분홍 삼각 코와 고양이 수염 */}
        <polygon points="100,83 96,88 104,88" fill="#f43f5e" />
        <path d="M 100 88 Q 94 92 90 90 M 100 88 Q 106 92 110 90" fill="none" stroke="#334155" strokeWidth="2" strokeLinecap="round" />

        {/* 양쪽 수염 */}
        <path d="M 64 82 L 48 80 M 64 88 L 46 90 M 136 82 L 152 80 M 136 88 L 154 90" fill="none" stroke="#94a3b8" strokeWidth="1.8" strokeLinecap="round" />

        {/* 앞발 2개 */}
        <g style={{ zIndex: 12 }}>
          <ellipse cx="78" cy="118" rx="11" ry="9" fill="#fff7ed" stroke="#fed7aa" strokeWidth="2" />
          <ellipse cx="122" cy="118" rx="11" ry="9" fill="#fff7ed" stroke="#fed7aa" strokeWidth="2" />
        </g>
      </g>
    </svg>
  );
...
function BedtimeSleepView() {
  ...
  const toggleLights = () => {
    ...
    if (nextState) {
      // 🌙 불을 끄면 평화로운 오르골 자장가만이 방 전체에 울려 퍼집니다 (TTS 충돌 방지)
  ...
  const handlePatAnimal = (animalId) => {
    ...
    // 🛌 핵심 규칙: 동물은 이불을 덮어줘야지만 잡니다!
    // 이불이 없는 상태에서는 아무리 토닥여도 잠들지 않고, 하품하며 이불을 덮어달라고 반응합니다.
    if (st.hasBlanket) {
      ...
      setTimeout(() => {
        setAnimalStates(prev => ({
          ...prev,
          [animalId]: { ...prev[animalId], yawn: false }
        }));
      }, 1600);
  ...
  const coverAnimalWithBlanket = (targetAnimalId, blanket) => {
    ...
    setTimeout(() => setAnimatingCoverId(null), 850);
  ...
  // 이불 개어주기 (다시 바구니로 복귀) - 이불이 벗겨지면 동물은 잠에서 깸
  ...
  return (
    <div style={{
          ...
          return (
            <div
              ...
              style={{
                ...
                transform: isTargeted ? 'scale(1.04)' : 'scale(1)',
                filter: isTargeted ? 'drop-shadow(0 0 25px rgba(250, 204, 21, 0.95))' : 'none',
            ...
            >
              ...
              {hearts.filter(h => h.animalId === animal.id).map(h => (
                <div
                  key={h.id}
                  style={{
                    position: 'absolute', top: '15px', fontSize: '2rem',
              ...
              {/* 🛏️ 프레임 없는 오픈 침실: 구름 베개 & 누워있는 동물 일러스트 */}
              <div
              ...
              >
                {/* 바닥의 은은하고 포근한 소프트 섀도우 (프레임 박스 대신 자연스러운 공간감) */}
                <div style={{
                  position: 'absolute', bottom: '10px', width: '85%', height: '36px',
                  background: 'radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0.18) 0%, transparent 75%)',
                  borderRadius: '50%', pointerEvents: 'none', zIndex: 1
                }} />

                {/* 1. 상단 푹신한 하얀 구름 베개 (Pillow Layer) */}
                <div style={{
                  position: 'absolute', top: '16px', width: '80%', height: '48px',
                  background: '#ffffff', borderRadius: '24px',
                  boxShadow: '0 6px 16px rgba(0,0,0,0.1), inset 0 2px 5px rgba(255,255,255,0.9)',
                  border: '1.5px solid rgba(226, 232, 240, 0.8)', zIndex: 2
                }}>
                  {/* 베개 퀼팅 스티치 선 */}
                  <div style={{
                    position: 'absolute', inset: '4px', borderRadius: '20px',
                    border: '1.5px dashed #cbd5e1', opacity: 0.55
                  }} />
                </div>

                {/* 2. 베개에 누워있는 동물 SVG 캐릭터 (얼굴이 이불 위로 100% 쏙 보임) */}
                <div style={{
                  width: '92%', height: '160px', position: 'relative', zIndex: 3,
                  marginTop: '6px'
                }}>
                  <AnimalComp isAsleep={st.isAsleep} hasBlanket={st.hasBlanket} />
                </div>
                ...
                {st.isAsleep && (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: '1.7rem', fontWeight: 900, color: '#c084fc',
                      textShadow: '0 2px 6px rgba(0,0,0,0.3)', pointerEvents: 'none', zIndex: 25
                    }}
                  >
                    zZ
                  </div>
                ...
                {/* 3. 🛏️ 동물에게 덮인 실제 퀼팅 이불 (글자 없는 순수 이불 형상, 얼굴 아래 몸통/배만 포근하게 덮임) */}
                {st.hasBlanket && (
                  <div
                    onClick={(e) => handleRemoveBlanket(animal.id, e)}
                    title="이불을 누르면 바닥으로 다시 펼쳐져요!"
                    className={`${activeBlanket.blanketClass || ''} ${isSnuggling ? 'blanket-tuck-anim' : ''}`}
                    style={{
                      position: 'absolute', bottom: '6px', left: '4%', right: '4%', height: '42%',
                      borderRadius: '16px 16px 26px 26px',
                      boxShadow: '0 -6px 18px rgba(0,0,0,0.22), inset 0 3px 6px rgba(255,255,255,0.4)',
                      zIndex: 10, overflow: 'hidden', cursor: 'pointer'
                    }}
                  >
                    {/* 상단 도톰한 뽀송뽀송 양털/플란넬 접힌 깃 (글자 없는 순수 깃) */}
                    <div style={{
                      position: 'absolute', top: 0, left: 0, right: 0, height: '16px',
                      background: 'linear-gradient(180deg, #ffffff 0%, #f8fafc 100%)',
                      borderBottom: '2px dashed rgba(148, 163, 184, 0.45)',
              ...
              {/* 하단 버튼 바: 토닥이기 & 이불 걷기 (틀에 박힌 프레임 없는 미니멀 스타일) */}
              <div style={{ display: 'flex', width: '100%', gap: '8px', zIndex: 10, marginTop: '4px' }}>
                {st.hasBlanket ? (
                  <button
                    onClick={(e) => handleRemoveBlanket(animal.id, e)}
                    style={{
                      flex: 1, padding: '6px 10px', borderRadius: '16px', border: 'none',
                      background: isLightsOff ? 'rgba(51, 65, 85, 0.8)' : 'rgba(255, 255, 255, 0.9)',
                      color: isLightsOff ? '#cbd5e1' : '#64748b',
                  ...
                  <div style={{
                    flex: 1, padding: '6px 8px', borderRadius: '16px',
                    background: isLightsOff ? 'rgba(30, 41, 59, 0.6)' : 'rgba(255, 255, 255, 0.7)',
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isLightsOff ? 'rgba(51, 65, 85, 0.8)' : 'rgba(255, 255, 255, 0.9)',
                    color: isLightsOff ? '#f8fafc' : '#b45309',
                    fontWeight: 900, fontSize: '0.78rem', cursor: 'pointer',
                    boxShadow: '0 2px 6px rgba(0,0,0,0.08)',
                    backdropFilter: 'blur(6px)'
                  }}
      ...
      <div style={{
        ...
        {/* 바닥에 자연스럽게 촤르륵 펼쳐진 3채의 실제 솜이불 (글자/프레임 없이 순수한 이불 형상) */}
        <div style={{
          display: 'flex',
          ...
          {SLEEP_ANIMAL_DATA.map((item, index) => {
            ...
            const spreadRotations = ['-3deg', '1.8deg', '-2.2deg'];
            const spreadRotation = spreadRotations[index % 3];

            if (isUsed) {
              // 이불을 동물에게 덮어준 경우: 바닥에는 이불이 놓여있던 포근한 바닥 매트 실루엣만 부드럽게 남음 (글자 없음)
              return (
                <div
                  key={item.blanketId}
                  style={{
                    ...
                    maxWidth: '280px',
                    height: '135px',
                    borderRadius: '36px 48px 30px 42px',
                    border: isLightsOff ? '2px dashed rgba(148, 163, 184, 0.2)' : '2px dashed rgba(251, 146, 60, 0.25)',
                    background: isLightsOff ? 'rgba(30, 41, 59, 0.2)' : 'rgba(255, 247, 237, 0.35)',
            ...
            return (
              <div
                ...
                style={{
                  ...
                  maxWidth: '290px',
                  height: '140px',
                  borderRadius: index === 0 ? '42px 56px 34px 48px' : index === 1 ? '48px 38px 52px 36px' : '38px 50px 36px 54px',
              ...
              >
                {/* 1. 상단 푹신하게 접힌 밍크/플란넬 깃 (글자 전혀 없이 순수한 도톰한 깃) */}
                <div className="duvet-collar" />

                {/* 2. 패브릭 주름 및 실제 솜이불 3D 볼륨 음영 오버레이 */}
                <div className="fabric-fold-shadow" />
              </div>
            );
          })}
      ...
      {/* 🎈 드래그 중인 플로팅 실제 이불 (글자 없는 순수 솜이불 형상으로 손가락을 따라 퐁신퐁신 떠다님) */}
      {draggingBlanket && (
        <div
          className={draggingBlanket.blanketClass || ''}
          style={{
            position: 'fixed',
            left: `${dragPos.x - 85}px`,
            top: `${dragPos.y - 55}px`,
            width: '170px',
            height: '105px',
            border: '3px solid rgba(255,255,255,0.9)',
            borderRadius: '24px 34px 20px 28px',
...
export default function App() {
  ...
  const openFeedModal = () => {
    ...
    audioEngine.playFreq(784, 'triangle', 0.12, 0.5);
